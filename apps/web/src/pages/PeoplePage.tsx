...
export function PeoplePage() {
  ...
  const updatePerson = useMutation({
    mutationFn: ({ id, payload }: { id: string; payload: Parameters<typeof api.updatePerson>[1] }) =>
  ...
  return (
    <div className="page">
      <div className="page-title-row">
        <div><span className="eyebrow">人物与关系</span><h1>我的家人</h1></div>
        <button className="button primary" onClick={openCreate}><Plus size={18} /> 添加家人</button>
      </div>

      {hasQueryIssue([people]) ? <QueryState queries={[people]} loadingText="正在读取人物档案……" /> : people.data?.length ? (
          ...
          {people.data.map((person) => (
            <article className="person-card" key={person.id}>
              ...
              <div className="person-details">
                ...
                <p>{person.birth_year ? `${person.birth_year} 年出生` : "出生年份待补充"}</p>
                <small>{person.birthplace || "故乡待补充"}</small>
              </div>
              <button className="icon-button" title="编辑人物" aria-label={`编辑${person.display_name}`} onClick={() => openEdit(person)}><Pencil size={17} /></button>
            </article>
          ))}
        </div>
      ) : (
        <EmptyState icon={UserRound} title="先添加一位想被记录的人" description="可以是自己、父母、祖辈，或任何重要的人。" />
      )}

      {open && (
        <div className="modal-backdrop" role="presentation" onMouseDown={() => setOpen(false)}>
          <form className="modal" role="dialog" aria-modal="true" aria-labelledby="person-dialog-title" onSubmit={submit} onMouseDown={(e) => e.stopPropagation()}>
            <span className="eyebrow">建立人物档案</span>
            <h2 id="person-dialog-title">{editingId ? "编辑人物档案" : "添加一位家人"}</h2>
            <label>姓名<input required value={name} onChange={(e) => setName(e.target.value)} placeholder="例如：林秀兰" /></label>
            <label>希望怎样称呼<input value={preferredName} onChange={(e) => setPreferredName(e.target.value)} placeholder="例如：林奶奶" /></label>
            <div className="form-grid">
              <label>出生年份<input inputMode="numeric" value={birthYear} onChange={(e) => setBirthYear(e.target.value)} placeholder="1952" /></label>
            ...
            <div className="modal-actions">
              ...
              <button className="button primary" disabled={createPerson.isPending || updatePerson.isPending}>保存人物</button>
            </div>
          </form>
        </div>
      )}
