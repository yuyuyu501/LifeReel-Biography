...
export function InterviewsPage() {
  ...
  const pastSessions = subjectSessions.filter((session) => (
    !session.chapter_id || chapterSessions.get(session.chapter_id)?.id !== session.id
  ));

  return (
    <div className="page">
      <div className="page-title-row">
        <div><span className="eyebrow">一章一段长期对话</span><h1>章节采访</h1></div>
      </div>

      {hasQueryIssue(requiredQueries) ? <QueryState queries={requiredQueries} loadingText="正在准备采访空间……" /> : !subject ? (
        <EmptyState icon={Mic2} title="添加人物后再开始采访" description="先告诉我们要记录谁，系统才能建立独立的生命记忆。" />
      ) : (
        <>
            ...
            {chapters.data?.map((chapter) => {
              ...
              <article className={`chapter-card interview-chapter-card ${existing ? `has-session ${existing.status}` : "not-started"}`} key={chapter.id}>
                ...
                <div className="chapter-saved-progress">
                  ...
                  {existing ? `已保存 ${answerCount} 轮回答` : "等待第一段讲述"}
                </div>
                <button
                  ...
                  onClick={() => existing
                    ...
                    : start.mutate({ subject_id: subject.id, chapter_id: chapter.id })}
                >
                  {isStarting
                    ? "正在建立记录"
                    : !existing
          ...
          {pastSessions.length > 0 && <section className="section-block compact">
            <div className="section-heading"><div><span className="eyebrow">历史存档</span><h2>过往采访</h2></div></div>
