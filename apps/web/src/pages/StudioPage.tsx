...
function matchesChapter(
  ...
  if (
    manifest?.scene_id === scene.id ||
    run.assets.some((asset) => asset.scene_id === scene.id)
  )
    return true;
  ...
  if (snapshot)
    return (
      snapshot.length === 1 &&
      !!scene.chapter_id &&
      snapshot[0].chapter_id === scene.chapter_id
    );
...
export function StudioPage() {
  ...
  const productionRuns = useQuery({
    ...
    queryFn: api.listProductionRuns,
  ...
  const terminalRuns = productionRuns.data
    ?.filter((run) => ["completed", "failed", "cancelled"].includes(run.status))
    .map(
      (run) =>
        `${run.id}:${run.status}:${run.updated_at}:${run.output_manifest?.billing?.status}`,
    )
    .join("|");
  useEffect(() => {
    if (terminalRuns) {
  ...
  const orderedRuns = [...(productionRuns.data ?? [])].sort(
    (left, right) =>
      new Date(right.created_at).getTime() -
      new Date(left.created_at).getTime(),
  );
  const requestedProject = scripts.data?.find(
    (project) => project.id === searchParams.get("project"),
  );
  const [effectiveSubjectId, setSubjectId] = usePageSubject(
    "studio",
    scripts.isPending ? undefined : people.data,
  ...
  const entries = (scripts.data ?? [])
    ...
    .flatMap((project) =>
      [...project.scenes]
        ...
        .map((scene) => ({ project, scene })),
    );
  const selected =
    entries.find(({ scene }) => scene.id === sceneId) ?? entries[0];
  const chapterRuns = selected
    ? orderedRuns.filter((run) =>
        matchesChapter(run, selected.project, selected.scene),
      )
    : [];
  const activeRun = chapterRuns[0];
  const planningIssue = ["VIDEO_PLAN_INVALID", "VIDEO_PLAN_FAILED"].includes(
    activeRun?.error_message ?? "",
  )
    ? activeRun?.output_manifest?.planning_diagnostics?.at(-1)?.issues[0]?.code
    : undefined;
  const productionError = statusLabel(
    planningIssue,
    statusLabel(activeRun?.error_message, "视频生成失败，请稍后重试。"),
  );
  const activeAsset = activeRun?.assets.find((asset) =>
    asset.mime_type.startsWith("video/"),
  );
  const completedSegments =
    activeRun?.output_manifest?.segments?.flatMap((segment, index) =>
      segment.status === "completed" ? [{ ...segment, index }] : [],
    ) ?? [];
  const selectedPartial =
    completedSegments.find(
      (segment) =>
        partial?.runId === activeRun?.id && partial?.index === segment.index,
    ) ?? completedSegments[0];
  const blocked =
    activeRun?.status === "failed" &&
    (Boolean(activeRun.recovery) ||
  ...
  const isGenerating = chapterRuns.some((run) =>
    ["queued", "running"].includes(run.status),
  );
  ...
  const quote =
    wallet.data?.prices && selected
      ? wallet.data.prices.video_billing_mode === "tokens" &&
        settings.data?.provider !== "mock"
        ? wallet.data.prices.video_reserve_cents
        : (segmented
            ? selected.scene.duration_seconds
            : (settings.data?.duration_seconds ??
              selected.scene.duration_seconds)) *
          wallet.data.prices.video_cents_per_second
      : undefined;
  ...
  const publish = useMutation({
    mutationFn: (id: string) => api.publish(id, "family"),
    onSuccess: () =>
  ...
  return (
    <div className="page film-studio-page">
      <header className="page-title-row studio-title">
        <div>
          <span className="eyebrow">人生影像</span>
          <h1>影像制作</h1>
        </div>
      ...
      {hasQueryIssue(queries) ? (
        <QueryState queries={queries} loadingText="正在准备影像制作台……" />
      ) : (
        <>
          <ErrorNotice
            error={
          ...
          <div className="studio-workspace">
            <aside className="studio-catalog" aria-label="制作章节">
              <label>
                ...
                >
                  {!people.data?.some((person) => person.is_subject) && (
                    <option value="">暂无家人</option>
                  )}
                  {people.data
                ...
                {entries.map(({ project, scene }, index) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={scriptEditing}
                      aria-current={
                        selected?.scene.id === scene.id ? "true" : undefined
                      }
                    ...
                    >
                      ...
                      <span>
                        ...
                        <small>剧本约 {scene.duration_seconds} 秒</small>
                        <small className="studio-chapter-status">
                          {run ? statusLabel(run.status) : "尚未生成"}
                        </small>
                      </span>
            ...
            <section className="studio-detail" aria-label="章节影像工作台">
              {selected ? (
                <>
                  <div className="studio-parameters">
                    <div className="studio-selection-heading">
                      ...
                      <span className="studio-mode-label">
                        {segmented ? "整章生成" : "试生成片段"}
                      </span>
                    </div>
                    <dl className="studio-specs" aria-label="视频参数">
                      <div>
                        <dt>目标画质</dt>
                        <dd>{settings.data?.resolution ?? "由服务决定"}</dd>
                      </div>
                      <div>
                        <dt>画面比例</dt>
                        <dd>{settings.data?.ratio ?? "由服务决定"}</dd>
                      </div>
                      <div>
                        <dt>目标时长</dt>
                        <dd>
                      ...
                      <div>
                        <dt>声音</dt>
                        <dd>
                          {settings.data?.generate_audio === true
                            ? "原生音频"
                            : settings.data?.generate_audio === false
                              ? "无声片段"
                              : "由服务决定"}
                        </dd>
                      </div>
                    ...
                    <div className="studio-generate-row">
                      <span className="studio-muted">
                        剧本预估 {seconds(selected.scene.duration_seconds)}
                      </span>
                      <Button
                        variant="default"
                  ...
                  <div className="studio-preview-toolbar">
                    ...
                    >
                      <button
                        ...
                        tabIndex={view === "video" ? 0 : -1}
                        onClick={() => setView("video")}
                      >
                        <Film size={17} aria-hidden="true" /> 视频预览
                      </button>
                      <button
                        id="studio-tab-script"
                        role="tab"
                        aria-selected={view === "script"}
                        aria-controls="studio-panel-script"
                    ...
                    {activeRun && (
                      <span
                        className={`run-status ${activeRun.status}`}
                        role="status"
                      >
                  ...
                  >
                    ...
                    {blocked &&
                      activeRun?.error_message === "VIDEO_CONTENT_REJECTED" && (
                        <Link to={`/scripts/${effectiveSubjectId}`}>
                          查看并修改剧本
                        </Link>
                      )}
                    {!activeAsset && selectedPartial && activeRun && (
                      <label className="studio-version">
                        已完成片段
                        <select
                          value={selectedPartial.index}
                          onChange={(event) =>
                            setPartial({
                              runId: activeRun.id,
                        ...
                        <span>
                          已完成 {completedSegments.length} /{" "}
                          {activeRun.output_manifest?.segments?.length}{" "}
                          段，整章尚未完成
                        </span>
                      </label>
                    )}
                    <div className="studio-screen">
                      {activeAsset ? (
                        <video
                          key={activeAsset.id}
                          aria-label={`${selected.scene.heading}视频`}
                          controls
                          playsInline
                          preload="metadata"
                          src={generatedAssetUrl(activeAsset.id)}
                          ...
                          onError={() => setMediaError(activeAsset.id)}
                        />
                      ) : selectedPartial && activeRun ? (
                        <video
                          key={`${activeRun.id}:${selectedPartial.index}`}
                          aria-label={`第 ${selectedPartial.index + 1} 段视频`}
                          controls
                          playsInline
                          preload="metadata"
                          src={productionSegmentUrl(
                            activeRun.id,
                            selectedPartial.index,
                          )}
                          onError={() =>
                            setMediaError(
                              `${activeRun.id}:${selectedPartial.index}`,
                            )
                          }
                        />
                      ) : (
                        <EmptyState
                          ...
                          description={isGenerating ? "等待生成结果" : ""}
                        />
                      )}
                    </div>
                    {activeAsset && mediaError === activeAsset.id && (
                      <div className="notice error" role="alert">
                        视频加载失败，请检查网络后重试。
                      </div>
                    )}
                    ...
                    {activeAsset && (
                      <div className="studio-output-meta">
                        <span>实际时长 {seconds(actualMedia?.duration)}</span>
                        <span>
                          实际尺寸{" "}
                          {actualMedia
                            ? `${actualMedia.width} × ${actualMedia.height}`
                            : "读取中"}
                        </span>
                        <span>
                    ...
                    {activeAsset && (
                      <div className="studio-publish-row">
                        {publication ? (
                          <>
                            <span className="studio-muted">已发布给家人</span>
                            <Button
                              variant="destructive"
                              size="sm"
                              className="button secondary small danger"
                              disabled={withdraw.isPending}
                              onClick={() => withdraw.mutate(publication.id)}
                            >
                              <Ban size={15} aria-hidden="true" /> 撤回发布
                            </Button>
                          </>
                        ) : (
                          <Button
                            ...
                            onClick={() =>
                              activeRun && publish.mutate(activeRun.id)
                            }
                          >
                            <Send size={16} aria-hidden="true" /> 发布给家人
                          </Button>
                        )}
                      </div>
                    )}
                  ...
                  <div
                    id="studio-panel-script"
                    role="tabpanel"
                    aria-labelledby="studio-tab-script"
                    hidden={view !== "script"}
                  >
                    <article className="studio-script">
                      <div className="studio-script-caption">
                        <span>当前剧本</span>
                        <span>
                          预估 {seconds(selected.scene.duration_seconds)}
                        </span>
                      </div>
                      <EditableScript
