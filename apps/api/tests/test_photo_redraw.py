...
def test_provider_uses_exact_prompt_and_downloads_without_credentials(monkeypatch):
    ...
    def handler(request):
        ...
        if request.method == "POST":
            ...
            assert "轻度彩色手绘转描" in payload["prompt"]
            assert "人物相似度优先于风格化" in payload["prompt"]
            assert "五官的相对位置和大小" in payload["prompt"]
            assert "不要卡通化" in payload["prompt"]
            assert "本图片由ai生成" not in payload["prompt"]
