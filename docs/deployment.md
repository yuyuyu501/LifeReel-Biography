...
选择原始照片并点击“轻度转描”。默认关闭；`mock` 仅允许开发和隔离测试环境使用。
当前固定使用 `Qwen/Qwen-Image-Edit-2509`、20 步和 `portrait-redraw-v3` 提示词，
要求人物相似度优先，仅做轻度彩色手绘，保留五官比例、年龄、发际线和辨识细节，避免卡通化。
视频各段沿用参考图特征，不继续强化卡通效果。密钥只存环境，不返回浏览器、不记录到任务中。

上传不会自动触发付费调用。用户点击轻度转描后创建数据库任务，由现有视频
...
衍生图不作为采访事实证据；转描不构成视频平台的真人肖像授权，视频提交仍需正常审核。
开启 `VIDEO_REFERENCE_STYLE=color_redraw` 后，生成本章影像会先准备剧本“形象”栏中的
图片，复用同版本图片编辑任务，再将转描图与所选音频提交给视频平台。编辑失败不会回退原图。
`color_redraw` 是兼容已有配置的历史名称；实际行为以 `reference_prompt_version` 为准。
新提示词不会复用旧转描或标注图；前端发现提示词版本变化时新建影像任务，保留历史任务与素材。
