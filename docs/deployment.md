...
选择原始照片并点击“彩色转绘”。默认关闭；`mock` 仅允许开发和隔离测试环境使用。
当前固定使用 `Qwen/Qwen-Image-Edit-2509`、20 步和版本化的二维彩色手绘提示词，
保留原有表情（不会要求所有人物闭眼）。密钥只存环境，不返回浏览器、不记录到任务中。

这是独立图片功能：上传不会自动触发付费调用。用户点击后创建数据库任务，由现有视频
...
当前转绘费用由运营方承担，不扣用户钱包，未纳入视频计费。

只接受已授权且不超过 10 MiB 的 PNG/JPEG/WebP 原图；调用前校验来源、哈希、文件类型和
人物归属。结果写入私有存储，保留来源、输出哈希、模型、提示词版本及服务商追踪 ID。
衍生图不作为采访事实证据，不自动或手动替代 Seedance 视频参考；转绘不构成视频平台的
真人肖像授权。后续视频使用必须另行接入目标平台认可的授权资产流程。
此功能复用现有素材和任务表，无新增数据库迁移；部署需重建 API 与 Web。
